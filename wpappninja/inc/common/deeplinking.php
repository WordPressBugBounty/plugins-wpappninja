...
function wpappninja_deeplink_process($buffer) {
	...
	$content = preg_replace_callback($regex, function ($matches) {
		...
		$scheme_http = parse_url( get_bloginfo('url'), PHP_URL_SCHEME );
		...
		if (preg_match('/^('.preg_quote(get_site_url(''), "/").'|\/[^\/])/', $url)) {
			if (!preg_match('#^http#', $url)) {
				...
				$home_url_domain = parse_url( get_bloginfo('url') , PHP_URL_SCHEME ) . '://' . parse_url( get_bloginfo('url') , PHP_URL_HOST ) . '/';
